...
%             dxf.bezier = zeros(3,dxf.ne);
            %plot(dxf);                                                         % Evtl. einbelenbden 
...
% plot polygons incl. bulges by vertices (X,Y)
    ...
    if isclosed==1
        ...
        %h = plot(abs(poly(:,1)),abs(poly(:,2)),'color',col);
    end
...
% plot arcs: (X Center,Y Center,Radius,Start angle,End angle)
    ...
    %h = plot(Xabs,Yabs,'-','color',col);
    h = plot(Xabs,Y,'-','color',col);
    %h = plot(X,Y,'-','color',col);
...
% plot lines: (Xi,Yi,Xj,Yj) start and end points 
    ...
    %h = plot([Xiabs Xjabs],[Yiabs Yjabs],'color',col);
